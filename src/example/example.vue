  ...
  <div class="main">
    <div class="top">
      ...
      <el-menu
        class="menu"
        style="border: none"
        :default-active="currentNavigationName"
        mode="horizontal"
        :ellipsis="false"
        active-text-color="#fff"
        text-color="#fff"
        :default-openeds="[currentNavigationName]"
      >
        <el-menu-item
        ...
        <div class="gitLink">
          ...
          <img
            @click="openUrl('gitee')"
            src="/files/site/gitee.png"
            alt="logo"
            width="36px"
            height="36px"
          />
          <img
            @click="openUrl('github')"
            src="/files/site/github.png"
            alt="logo"
            width="36px"
            height="36px"
          />
    ...
    <div class="center">
      <div class="nav">
        <el-menu
          class="menu"
          style="border: none"
          :default-active="currentClassify"
          :ellipsis="false"
          text-color="#fff"
          active-text-color="#71a5ee"
        >
          <el-menu-item
            :class="{ menuItem: item.pid == currentClassify }"
            v-for="item in data.raw_classify_list"
            :key="item.pid"
            :index="String(item.pid)"
        ...
        <div class="navigation-parent">
          <div class="navigation" ref="navigationRef">
            <div v-for="examples in data.classify_list">
              <div
                :to="examples.pid"
                style="position: relative; z-index: 1; top: -40px"
              ></div>
...
let currentNavigationName =
  localStorage.getItem("navigation") || navigation_list[0].name;

data.raw_classify_list = (
  navigation_list.find((item) => item.name === currentNavigationName) ||
  navigation_list[0]
).examples;
...
let currentClassify = ref(
  localStorage.getItem("classify") || data.raw_classify_list[0].pid
);
...
const goNavigation = async (item) => {
  ...
  const findClassify = item.examples.find(
    (item) => item.pid === currentClassify.value
  );
...
onMounted(() =>
  navigationRef.value
    .querySelector(`[to="${currentClassify.value}"]`)
    ?.scrollIntoView()
);
...
const showCode = (item, examples) => {
  ...
  const path = router.resolve({
    ...
    query: {
      navigation: currentNavigationName,
      classify: examples.pid,
      id: item.id,
    },
  }).href;
  ...
  &-item {
    ...
    .box {
      ...
      .image {
        ...
        &:hover {
          box-shadow: rgba(0, 0, 0, 0.38) 0px 6px 12px,
