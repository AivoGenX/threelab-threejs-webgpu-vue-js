<template>
  ...
  <div class="lico" :style="{ left: expand ? 'calc(40vw - 30px)' : '0px' }">
...
let currentExample = list
  .find((item) => item.name === query.navigation)
  ?.examples.find((item) => item.pid === query.classify)
  ?.children.find((i) => i.id === query.id);

if (!currentExample) {
  const randomExample1 =
    list[0].examples[Math.round(Math.random() * (list[0].examples.length - 1))];

  currentExample =
    randomExample1.children[
      Math.round(Math.random() * (randomExample1.children.length - 1))
    ];
}
